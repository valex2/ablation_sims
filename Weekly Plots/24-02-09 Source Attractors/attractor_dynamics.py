...
"""The goal of this file is to determine whether or not the network's source seperation
exhbiits clear attractor dynamics in state space. That is, for a given source, the PC space dynamics
of the population exhibit a clear convergence around a fixed point. This is important for understanding
the network's ability to maintain a stable representation of the input over time, and how we might
modify that representation post-lesioning to yield similar behavior"""
...
name = "4 Source Seperation" # NOTE: change this for the name of the experiment
...
electrode_number = 64 # NOTE total number of stimulation sites
source_number = 4 # NOTE number of inputs we're trying to seperate from
...
ablation_frac = 0.95 # fraction of neurons to ablate
...
with nengo.Simulator(model) as sim:
    ...
    #-----------------------------------------------------------------------------------------------#
    ###### NOTE: state space plotting (together) ######
    #--------------------------------------------------------------------------------------------------#
    ...
    num_time_steps = pre_neural_state_space.shape[0] # Create a colormap based on the number of time steps
    ...
    for i in range(source_number):
        ...
        for j in range(num_time_steps):
            if source_active[j] == 1:
                color_weights[j] += 2**i  # Encode active sources as bits in an integer
    
    # color_choices = [ # these align with the colors in the mixed input plot
    #     "#000000",  # Black -- this is the off color
    #     "#FFA500",  # Orange -- source 1 is on
    #     "#800080",  # Purple -- source 2 is on
    #     "#800000",  # Maroon -- source 1 and 2 are on
    #     "#008000",   # Green
    #     "#FF0000",  # Red
    #     "#0000FF",  # Blue
    #     "#800080",  # Purple
    ...
    color_choices = ["#000000"] + [mpl.colors.to_hex(mpl.cm.tab10(i)) for i in range(10)] + ["#FF00FF", "#FFC0CB", "#800000", "#008080", "#FFA500", "#800080"]

    colors = [color_choices[int(weight)] for weight in color_weights]  # Map encoded state to colors
    # colors = []
    # for color_weight in color_weights[j]:
    #     colors.append(color_choices[int(color_weight)])

    for i in range(num_time_steps): # plot the points with gradient coloring
        x = pre_neural_state_space[i, 0]
        y = pre_neural_state_space[i, 1]
        z = pre_neural_state_space[i, 2]
    ...
    ##### NOTE: plane plotting #####
    fit, residual = plane_of_best_fit(pre_neural_state_space)
    xmin, xmax = np.min(pre_neural_state_space[:, 0]), np.max(pre_neural_state_space[:, 0]) # specify linspace overwhich to plot the plane based on datapoints
    ymin, ymax = np.min(pre_neural_state_space[:, 1]), np.max(pre_neural_state_space[:, 1])
    ...
    # marker = line_integral(pre_neural_state_space)
    # ax.plot(pre_neural_state_space[0:(num_time_steps - 1),0], pre_neural_state_space[0:(num_time_steps - 1),1], pre_neural_state_space[0:(num_time_steps - 1),2], c = "blue", label = f"pre-ablation trajectory", alpha = 0.4) # plot the trajectory
    ax.plot(pre_neural_state_space[0:(num_time_steps - 1),0], pre_neural_state_space[0:(num_time_steps - 1),1], pre_neural_state_space[0:(num_time_steps - 1),2], c = "blue", alpha = 0.4) # plot the trajectory
    ...
    for i in range(num_time_steps - 1): # plot the points with gradient coloring
        x = post_neural_state_space[i, 0]
        y = post_neural_state_space[i, 1]
        z = post_neural_state_space[i, 2]
        ax.scatter(x, y, z, c=colors[i], s=10)
    # marker = line_integral(post_neural_state_space)
    
    ##### NOTE: plane plotting #####
    fit, residual = plane_of_best_fit(post_neural_state_space)
    ...
    ##### NOTE: compute the difference in angle -- this isn't being used at the moment #####
    ...
    # ax.plot(post_neural_state_space[0:(num_time_steps - 1),0], post_neural_state_space[0:(num_time_steps - 1),1], post_neural_state_space[0:(num_time_steps - 1),2], c = "red", label = f"post-ablation trajectory", alpha = 0.4) # plot the trajectory
    ax.plot(post_neural_state_space[0:(num_time_steps - 1),0], post_neural_state_space[0:(num_time_steps - 1),1], post_neural_state_space[0:(num_time_steps - 1),2], c = "red", alpha = 0.4) # plot the trajectory

    if source_number == 1:
        color_labels = ['off', 'S1']  # Adjust labels as needed
    elif source_number == 2:
        color_labels = ['off', 'S1', 'S2', 'S1 and S2']
    elif source_number == 3:
        color_labels = ['off', 'S1', 'S2', 'S3', 
                        'S1 and S2', 'S1 and S3', 'S2 and S3', 'S1, S2, and S3']
    elif source_number == 4:
        color_labels = ['off', 'S1', 'S2', 'S3', 'S4', 
                        'S1, S2', 'S1, S3', 'S1, S4', 
                        'S2, S3', 'S2, S4', 'S3, S4', 
                        'S1, S2, S3', 'S1, S2, S4', 'S1, S3, S4', 
                        'S2, S3, S4', 'S1, S2, S3, S4']

    color_patches = [mpatches.Patch(color=color_choices[i], label=color_labels[i]) for i in range(len(color_labels))]
    ax.legend(handles=color_patches, prop={'size': 8})
    ...
    #--------------------------------------------------------------------------------------------------#
    ###### NOTE: state space plotting (pre-ablation only) ######
    #--------------------------------------------------------------------------------------------------#

    pre_state_space = plt.figure() # Create a 3D plot
    ax = pre_state_space.add_subplot(111, projection='3d')
    ax.set_xlabel('PC 1', labelpad=-2) # Set labels for the axes
    ax.set_ylabel('PC 2', labelpad=-2)
    ax.set_zlabel('PC 3', labelpad=-2)
    ax.set_xticks([]) # remove the tick marks from the axis
    ax.set_yticks([])
    ax.set_zticks([])
    ax.set_title("State Space (Pre-Ablation)")
    cmap = plt.get_cmap('Blues')
    ...
    ax.view_init(elev=33, azim=11) # rotate the plot
    pre_state_space.savefig(path + "/pre_ablation_state_space_rot_1", dpi = 300)
    ax.view_init(elev=46, azim=138) # rotate the plot
    pre_state_space.savefig(path + "/pre_ablation_state_space_rot_2", dpi = 300)

    #--------------------------------------------------------------------------------------------------#
    ###### NOTE: state space plotting (post-ablation only) ######
    #--------------------------------------------------------------------------------------------------#
    post_state_space = plt.figure() # Create a 3D plot
    ax = post_state_space.add_subplot(111, projection='3d')
    ax.set_xlabel('PC 1', labelpad=-2) # Set labels for the axes
    ax.set_ylabel('PC 2', labelpad=-2)
    ax.set_zlabel('PC 3', labelpad=-2)
    ax.set_xticks([]) # remove the tick marks from the axis
    ax.set_yticks([])
    ax.set_zticks([])
    ax.set_title("State Space (Post-Ablation)")
    cmap = plt.get_cmap('inferno')
    ...
    ##### NOTE: zooming into the ablated population #####
    x_width = np.max(abs(post_neural_state_space[:, 0])) # Get the maximum x value
    y_width = np.max(abs(post_neural_state_space[:, 1])) # Get the maximum y value
    ax.set_xlim(-1*x_width, x_width)  # Zoomed x limits
    ax.set_ylim(-1*y_width, y_width)  # Zoomed y limits

    ax.legend(handles=color_patches)
    ax.view_init(elev=33, azim=11) # rotate the plot
    post_state_space.savefig(path + "/post_ablation_state_space_rot_1", dpi = 300)
    ax.view_init(elev=46, azim=138) # rotate the plot
    post_state_space.savefig(path + "/post_ablation_state_space_rot_2", dpi = 300)
    ...
    colors = plt.cm.viridis(np.linspace(0, 1, source_number)) # Colors for each source

    x_labels = ['Pre', 'Post']  # pre and post axis labels
    x_indices = np.arange(len(x_labels))  # This will be 0 and 1 for 'Pre' and 'Post'

    for i in range(source_number):  # Plotting lines for pre-ablation and post-ablation for each source
        plt.plot(x_indices, [pre_source_performance[i], post_source_performance[i]], color=colors[i], marker='o', linestyle='--', label=f"Source {i+1}")
    plt.plot(x_indices, [pre_accuracy, post_accuracy], color='black', marker='o', label='Cumulative')
...
axs[0].imshow(plt.imread(path + "/state_space_rot_2.png"), aspect='auto')
axs[1].imshow(plt.imread(path + "/pre_ablation_state_space_rot_2.png"), aspect='auto')
axs[2].imshow(plt.imread(path + "/post_ablation_state_space_rot_2.png"), aspect='auto')

axs[3].imshow(plt.imread(path + "/state_space_rot_1.png"), aspect='auto')
axs[4].imshow(plt.imread(path + "/pre_ablation_state_space_rot_1.png"), aspect='auto')
axs[5].imshow(plt.imread(path + "/post_ablation_state_space_rot_1.png"), aspect='auto')

axs[6].imshow(plt.imread(path + "/mixture_input.png"), aspect='auto')
axs[7].imshow(plt.imread(path + "/pre_ablation_spike_raster.png"), aspect='auto')
