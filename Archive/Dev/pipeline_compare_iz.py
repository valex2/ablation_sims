...
for ablation_frac in ablation_frac_range:
    ...
    for iter in range(1):
        ...
        with model: 
            ...
            exc_pop = nengo.Ensemble(n_neurons = num_exc_neur, dimensions = 1, seed = rand_num, neuron_type = nengo.Izhikevich())
            inh_pop = nengo.Ensemble(n_neurons = num_inh_neur, dimensions = 1, neuron_type = nengo.Izhikevich())
            null_pop = nengo.Ensemble(n_neurons = num_exc_neur, dimensions = 1, seed = rand_num, neuron_type = nengo.Izhikevich()) # baseline to see whether signal is reproducible given population size
...
mse_path = os.path.join(path, 'iz_null')  # Main directory
...
mse_path = os.path.join(path, 'iz_joint')  # Main directory
if not os.path.exists(mse_path):
    os.makedirs(mse_path)
