...
ablation_frac_range = np.linspace(0, 0.9, 300) # TODO this is the parameter space that we range across
ablation = ablation_frac_range[ARRAY_ID] # pull this based off of the run ID
...
print(f"\n\n\n THIS IS CHILD {ARRAY_ID} with ablation {ablation:2f} out of {ablation_frac_range} \n\n\n")
...
slurm_dict =  {
              'num_exc_neur' : 100,
              'num_inh_neur' : 20,
              'ablation_frac' : ablation, # fraction of neurons to lesion
...
with nengo.Simulator(model) as sim:
    ...
    for subdirectory_name, data in subdirectories:
        ...
        file_path = os.path.join(subdirectory_path, f"job_{ARRAY_ID}_ablation_{ablation:.2f}_{subdirectory_name}.csv")
        np.savetxt(file_path, data, delimiter=',', fmt='%1.3f')

    # if not os.path.exists('povs'):
    #     os.makedirs('povs')
    # povs_path = os.path.join()

    # if not os.path.exists('povs_pre'):
    #     os.makedirs('povs_pre')
    # temp_path = os.path.join(path, 'povs_pre')
    # np.savetxt(os.path.join(temp_path, f"{ARRAY_ID}_pre.csv"), povs_pre, delimiter=',',fmt='%1.3f')

    # if not os.path.exists('povs_joint_pre'):
    #     os.makedirs('povs_joint_pre')
    # temp_path = os.path.join(path, 'povs_joint_pre')
    # np.savetxt(os.path.join(temp_path, f"{ARRAY_ID}_joint_pre.csv"), joint_povs_pre, delimiter=',',fmt='%1.3f')

    # if not os.path.exists('povs_null'):
    #     os.makedirs('povs_null')
    # temp_path = os.path.join(path, 'povs_null')
    # np.savetxt(os.path.join(temp_path, f"{ARRAY_ID}_null.csv"), null_povs, delimiter=',',fmt='%1.3f')

    # if not os.path.exists('povs_post'):
    #     os.makedirs('povs_post')
    # temp_path = os.path.join(path, 'povs_post')
    # np.savetxt(os.path.join(temp_path, f"{ARRAY_ID}_post.csv"), povs_post, delimiter=',',fmt='%1.3f')

    # if not os.path.exists('povs_joint_post'):
    #     os.makedirs('povs_joint_post')
    # temp_path = os.path.join(path, 'povs_joint_post')
    # np.savetxt(os.path.join(temp_path, f"{ARRAY_ID}_joint_post.csv"), joint_povs_post, delimiter=',',fmt='%1.3f')
