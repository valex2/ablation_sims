...
for ablation_frac in ablation_frac_range:
    ...
    for iter in range(1):
        ...
        with nengo.Simulator(model) as sim:
            ...
            ### compute the procrustes similiarity between pre and post ablation ###
            procrustes_similarity  = spatial.procrustes(pre_neural_state_space, post_neural_state_space)[2]
            ...
            proscrustes_iter.append(procrustes_similarity)
            
        null_mses[ablation_frac] = null_iter
        conn_mses[ablation_frac] = conn_iter
        proscrutes[ablation_frac] = proscrustes_iter

mse_path = os.path.join(path, 'null_mse_5')  # Main directory
...
mse_path = os.path.join(path, 'conn_mse_5')  # Main directory
if not os.path.exists(mse_path):
    os.makedirs(mse_path)
...
mse_path = os.path.join(path, 'proscrutes_similarity')  # Main directory
if not os.path.exists(mse_path):
    os.makedirs(mse_path)
...
with open(file_path, 'w', newline='') as csv_file:
    writer = csv.DictWriter(csv_file, fieldnames=proscrutes.keys())
    
    writer.writeheader() # Write the header
    
    for row in zip(*proscrutes.values()): # Write the rows
        writer.writerow(dict(zip(proscrutes.keys(), row)))
